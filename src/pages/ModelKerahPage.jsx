...
export default function ModelKerahPage({ setCurrentPage }) {
    const [kerahTypes, setKerahTypes] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        const fetchKerah = async () => {
            const { data, error } = await supabase.from('page_collars').select('*').order('created_at', { ascending: true });
            if (error) console.error("Error loading kerah:", error);
            else setKerahTypes(data || []);
            setLoading(false);
        };
        fetchKerah();
    }, []);
    ...
    return (
        <div className="pt-32 pb-20 bg-black min-h-screen">
            <SEO title="Model Kerah Jersey" description="Berbagai macam referensi model kerah jersey olahraga. Pilih desain terbaik yang sesuai dengan kebutuhan tim Anda." />
            <div className="container mx-auto px-6">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
                    {loading ? (
                        ...
                        kerahTypes.map((k) => (
                            <div key={k.id || k.name} className="group p-7 bg-neutral-900 border border-white/5 hover:border-white/30 transition-all duration-300 relative">
                                {k.is_popular && (
                                ...
                                <div className="w-full h-36 bg-black border border-white/10 flex items-center justify-center mb-5 overflow-hidden">
                                    {k.image_url ? (
                                        <img src={k.image_url} alt={k.name} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
                                    ) : (
                                        <div className="relative">
                                            ...
                                            {(k.name.includes('Half-Zip') || k.name.includes('Full Zip') || k.name.includes('Zip')) && (
                                ...
                                <p className="text-gray-400 text-sm leading-relaxed font-light">{k.description}</p>
                            </div>
                        ))
                    )}
                </div>
